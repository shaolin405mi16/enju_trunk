...
/*
 * bit modified by Kohji 'Shaolin' Matsubayashi of Vine Caves, Ltd.
 */

(function($) {

    $.fn.setCalendar = function(opts) {

        this.each(function(elm) {

            var fieldId = this.id;

/* VC: add bits to change date format temporarily -------- */
            if (opts && 'displayTime' in opts) {
                var displayTime = opts.displayTime;
            } else {
...
/* VC: -------- */
            ...
            if (opts && 'src' in opts) {
                ...
                Calendar.setup({inputField : fieldId, dateFormat : dateFmt, ifFormat : dateFmt, daFormat : dateFmt, showsTime: displayTime, button : triggerId });
            } else {
                Calendar.setup({inputField : fieldId, ifFormat : dateFmt, daFormat : dateFmt, showsTime: displayTime, button : fieldId });
            }
...
Date.parseDate = function(str, fmt) {
	...
	var d = 0;
	var a = str.split(/\W+/);
/* VC: add dirty bits to handle '%Y%m%d' format -------- */
        if (a.length < 2) {
            a = [ str.substr(0,4), str.substr(4,2), str.substr(6,2) ] ;
        }
/* VC: -------- */
	var b = fmt.match(/%./g);
	var i = 0, j = 0;
...
/**
 * -----------------------------------------------------------
 * below code is originally from calender/calendar-ja.js
 */
...
/**
 * -----------------------------------------------------------
 * below code is originally from calender/calendar-setup.js
 */
